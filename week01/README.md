# week01：ETH/UCY 数据格式梳理与五场景轨迹可视化

这一周做了两件事：

1. **把 ETH/UCY 的数据摸清楚** —— 五个在范围内的场景、哪些文件要用、哪些用不到、坑在哪；
2. **每个场景画一张轨迹图**，先看清楚数据长什么样。

第 1 件是本周的主要产出，也是后面所有工作的地基，所以整节放在下面。
...
以下两条在 2026-09-11 实测确认过。

### 1. 帧步长不统一

帧步长（见上面场景表）五个场景不一致，所以 `seq_eth` 的帧号与其他序列不在同一时间单位上。
...
week01/
├── README.md              本文件
├── plot_trajectories.py   绘图脚本（数据读 datasets/，产物写 results/）
